// Firewall policy compiler shipped before the change advisory board's controls review.
package main
...
func main() {
	...
	maxRules := int(pol.Default["max_rules"])
	portCeiling := int(pol.Default["port_ceiling"])
	lookback := int(pol.Default["max_shadow_lookback"])
	denySequence := int(pol.Default["default_deny_sequence"])
	...
	for _, r := range rules {
		...
		if len(dst) == 0 {
			dst = []prefix{{0, 0}}
			r.DestinationCIDRs = []string{"0.0.0.0/0"}
		}
		...
		if shadowedBy != "" {
			shadowedCount++
			queue = append(queue, queueRow{r.RuleID, "shadowed"})
			continue
		}
